...
@Component({
  selector: 'app-prestiti-create',
  templateUrl: './prestiti-create.component.html',
  styleUrls: ['./prestiti-create.component.css']
})
export class PrestitiCreateComponent implements OnInit {
  ...
  create(utente_id: string, libro_id: string, data_prestito : string, data_riconsegna : string) {
    ...
    this.prestitiservice.create(utente_id, libro_id,data_prestito,data_riconsegna ).subscribe((response : any) => {
      console.log(response);
      this.prestiti = response;
      this.users = response;
      alert('prestito aggiunto')
    });
  }
